...
def _make_data_from_raw_catalog_material(db: dbTolls, raw_catalog_row: sqlite3.Row,
                                         item: DirectoryItem, origin: int) -> tuple | None:
    ...
    if raw_parent_id is None:
        origin_row = get_origin_row_by_id(db, origin)
        parent_id = get_catalog_id_by_origin_code(db=db, origin=origin, code=origin_row['name'])
    else:
        ...
        parent_id = get_catalog_id_by_origin_code(db=db, origin=origin, code=parent_code) if parent_code else None
...
def _update_material_catalog(db: dbTolls,
                             catalog_id: int, raw_table_row: sqlite3.Row,
                             item: DirectoryItem, origin: int
                             ) -> int | None:
    """ Формирует строку из Сырой таблицы. Изменяет catalog_id запись в таблице Каталога. """
    data = _make_data_from_raw_catalog_material(db, raw_table_row, item, origin)
    # FK_tblCatalogs_tblOrigins, ID_parent, period, code, description, FK_tblCatalogs_tblItems, ID_tblCatalog
    data = (origin, *data, catalog_id)
...
def _insert_material_raw_catalog(db: dbTolls,
                                 raw_table_row: sqlite3.Row, item: DirectoryItem, origin: int
                                 ) -> int | None:
    """ Формирует строку из Сырой таблицы. Вставляет новую запись в таблицу Каталога. """
    # FK_tblCatalogs_tblOrigins, ID_parent, period, code, description, FK_tblCatalogs_tblItems, ID_tblCatalog
    data = (origin, ) + _make_data_from_raw_catalog_material(db, raw_table_row, item, origin)
...
def _transfer_raw_item_to_catalog(db_filename: str, item: DirectoryItem, catalog_name: str):
        ...
        for row in raw_data:
            ...
            if catalog_row:
                ...
                if raw_period >= row_period:
                    changed_count = _update_material_catalog(db, row_id, row, item, origin_id)
                    if changed_count:
                ...
                work_id = _insert_material_raw_catalog(db, row, item, origin_id)
                if work_id:
...
def transfer_raw_data_to_catalog(db_name: str, directory: str, catalog_name: str, main_code: str):
    ...
    for item in dir_catalog[1:]:
        _transfer_raw_item_to_catalog(db_name, item, catalog_name=catalog_name)
