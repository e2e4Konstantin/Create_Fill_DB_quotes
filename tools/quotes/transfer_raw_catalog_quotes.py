...
def _get_directory_id(item_name: str, db: dbTolls) -> int | None:
    ...
    id_catalog_items = db.get_row_id(
        sql_items_queries["select_items_team_code"],
        ( directory_name, item_name, ),
    )
    if id_catalog_items is None:
        output_message(
            f"В {directory_name} справочнике не найден:", f"название: {item_name!r}"
        )
...
def _make_data_from_raw_quotes_catalog(
    ...
    if raw_parent_code is None:
        ...
        parent_id = get_catalog_id_by_origin_code(
            db=db, origin=origin_id, code=parent_code
        )
    if parent_id and str(item.id):
        ...
        output_message_exit(
            f"В Каталоге не найдена родительская запись с шифром: {raw_parent_code!r}",
...
def transfer_raw_quotes_to_catalog(
    db_file_name: str, catalog_name: str, period_id: int
):
    """
    ...
    for chapter in chapters:
        delete_catalog_old_period_for_parent_code(
            db_file_name, origin=origin_id, parent_code=chapter[0]
        )
