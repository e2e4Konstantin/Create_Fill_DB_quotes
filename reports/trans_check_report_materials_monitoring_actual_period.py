...
def _header_create(table: list[Material], view_history_depth: int) -> str:
    header = [
        ...
        "monitoring price",
        #
        "history freight included",
        "history freight not included",
        "history check",
        #
        "transport flag",
        "transport code",
...
def _material_row_create(material: Material, row_number: int, max_history_len: int):
    ...
    #
    pos["index_num"] = [history_end_column + 1, material.index_num]
    pos["actual_price"] = [history_end_column + 2, material.actual_price]
    pos["monitoring_index"] = [history_end_column + 3, material.monitoring_index]

    pos["monitoring_price"] = [history_end_column + 4, material.monitoring_price]
    pos["history_freight_included"] = [history_end_column + 5, material.history_freight_included]
    pos["history_freight_not_included"] = [history_end_column + 6, material.history_freight_not_included]
    pos["history_check"] = [history_end_column + 7, material.history_check]
    #
    pos["transport_flag"] = [history_end_column + 8, material.transport_flag]
    pos["transport_code"] = [history_end_column + 9, material.transport_code]
    pos["transport_base_price"] = [history_end_column + 10, material.transport_base_price]

    pos["transport_numeric_ratio"] = [history_end_column + 11, material.transport_numeric_ratio]

    pos["transport_actual_price"] = [history_end_column + 12, material.transport_actual_price]
    #
    pos["gross_weight"] = [history_end_column + 13, material.gross_weight]
    pos["empty"] = [history_end_column + 14, ""]
    ...
    # formulas = [
    #     f"={pos["transport_base_price"][0]}{row_number}*{pos['transport_numeric_ratio'][0]}{row_number}*{pos['gross_weight'][0]}{row_number}/1000",
    #     f"=ROUND(IF({pos['transport_flag'][0]}{row_number}, ({pos['monitoring_price'][0]}{row_number}-{pos['transport_price'][0]}{row_number}), {pos['monitoring_price'][0]}{row_number}),2)",
    #     f"={pos['actual_price'][0]}{row_number}/{pos['base_price'][0]}{row_number}",
    #     f"={pos['result_price'][0]}{row_number}/{pos['base_price'][0]}{row_number}",
    #     f"={pos['result_index'][0]}{row_number}-{pos['previous_index'][0]}{row_number}",
    #     f"=({pos['result_index'][0]}{row_number}/{pos['previous_index'][0]}{row_number})-1",
    #     "",
    #     material.abbe_criterion,
    #     f"=ROUND(ABS({pos['result_price'][0]}{row_number}-{pos['actual_price'][0]}{row_number}),3)",
    #     f'=IF({pos["absolute_price_change"][0]}{row_number}<=1.4*{pos["abbe_criterion"][0]}{row_number}, "", 1)',
    #     f"=({pos['result_price'][0]}{row_number}/{pos['actual_price'][0]}{row_number})-1",
    # ]
    # mod_row = [*data_pos, *formulas]
    # ic(mod_row)
    # return mod_row
