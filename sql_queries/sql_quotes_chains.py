sql_quotes_chain_create = {
    ...
    "create_table_quotes_chains": """
       CREATE TABLE IF NOT EXISTS tblChains (
            ID_Chain   INTEGER PRIMARY KEY NOT NULL,
            FK_tblChains_tblProducts_Parent INTEGER DEFAULT NULL,                   -- родительская продукт
            FK_tblChains_tblProducts_Child  INTEGER DEFAULT NULL,                   -- подчиненный продукт
            last_update         INTEGER NOT NULL DEFAULT (UNIXEPOCH('now')), -- время последнего обновления
            FOREIGN KEY (FK_tblQuotes_child) REFERENCES tblProducts (ID_tblProduct),
            FOREIGN KEY (FK_tblQuotes_parent) REFERENCES tblProducts (ID_tblProduct),
